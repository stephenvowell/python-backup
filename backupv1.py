"""
at a command prompt, run the following commands to install the required packages:
pip install shutil
pip install filecmp
pip install schedule
pip install tkinter

if you haveing an error with the tkinter package, you can install the following package:
sudo apt-get install python3-tk


This script is a backup scheduler that allows you to copy files and folders from a source directory to a destination directory at a specified interval.
Functions:
- copy_folder(src, dst): Copies files and folders from the source directory to the destination directory.
- start_backup(src, dst, interval, time_str): Starts the backup process by scheduling the copy_folder function to run at a specified interval and time.
- browse_folder(entry): Opens a file dialog to select a folder and inserts the selected folder path into the given entry widget.
- validate_time_format(time_str): Validates the format of a time string (HH:MM).
- on_start(): Retrieves the values from the entry widgets, validates them, and starts the backup process in a separate thread.
"""
...
log_file = os.path.join(log_dir, 'copy_errors.log')
logging.basicConfig(filename=log_file, level=logging.ERROR, 
                    format='%(asctime)s:%(levelname)s:%(message)s')

# Configure a separate logger for successful copies
success_log_file = os.path.join(log_dir, 'copy_success.log')
...
root = tk.Tk()
root.title("Backup Scheduler for Python")
